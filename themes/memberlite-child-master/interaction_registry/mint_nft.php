...
function pte_get_registry_mint_nft() {
  ...
  $registryArray = array(
      'mint_nft' => function(Token $token) {
            ...
            if (!isset($requestData['nft_media_is_processing'])) {
              wsc_start_nft_media_processing($requestData);
              $requestData['nft_media_is_processing'] = true;
            }

            $nftMediaIsProcessing = isset($requestData['nft_media_is_processing']);
            $nftReadyToMint = $token->getValue("ready_to_mint") ? $token->getValue("ready_to_mint") : false;

            if ($nftReadyToMint) {    // TODO Why is this not in requestdata by now?
